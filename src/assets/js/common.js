...
export const filterLabel = (val, list) => {
  if ((val !== 0 && !val) || !list || list.length === 0) return '--';
...
export const formData = (val, type="YYYY-MM-DD HH:mm:ss") => {
  if (val) {
...
export function removeProperty(obj) {
  Object.keys(obj).forEach(item => {
    if (obj[item] === '' || obj[item] === undefined || obj[item] === null || obj[item] === 'null' || JSON.stringify(obj[item]) === '{}') delete obj[item]
  })
  return obj
}
